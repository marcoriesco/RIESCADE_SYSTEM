...
function App() {
    ...
    const updateControllers = () => {
      ...
      const active = Array.from(gamepads)
        ...
        .map(gp => {
          ...
          } else if (id.toLowerCase().includes('xinput') || id.toLowerCase().includes('xbox 360')) {
      ...
      if (active.length > 0) window.api.executeCommand('set-active-controllers', active)
    }
  ...
  const themeData = useMemo(() => {
    ...
    const baseData: any = {
      ...
      'system:theme': sys?.theme || sys?.name || 'auto-allgames',
      'system:hardwareType': sys?.hardware || 'console',
      'global:time': new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
    ...
    if (selectedSystem && currentGame) {
      ...
      return {
        ...
        'game:titleshot': resolveMedia(currentGame.titleshot || currentGame.image),
        'game:wheel': resolveMedia(currentGame.wheel || currentGame.marquee || currentGame.image),
      ...
      if (!selectedSystem) {
        ...
        if (e.key === 'PageUp') {
          ...
          while (next !== systemIndex) {
            if ((systems[next]?.hardware || '') !== currentHw) { setSystemIndex(next); break }
        ...
        if (e.key === 'PageDown') {
          ...
          while (prev !== systemIndex) {
            if ((systems[prev]?.hardware || '') !== currentHw) {
              ...
              let first = prev
              while (first > 0 && (systems[first - 1]?.hardware || '') === targetHw) first--
  ...
  return (
    <div className="app-root" style={{ width: '100vw', height: '100vh', overflow: 'hidden', background: '#000' }}>
      ...
      />
      <Menu isOpen={isMenuOpen} onClose={() => setIsMenuOpen(false)} theme={theme} themeData={themeData} />
    </div>
  )
}
