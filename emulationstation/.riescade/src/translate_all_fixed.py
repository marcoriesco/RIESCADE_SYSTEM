...
def translate_individual(text, target_lang):
    try:
        ...
        print(f"Error translating individual '{text}' to {target_lang}: {e}")
        return text
...
def translate_batch(texts, target_lang):
    ...
    combined_text = "\n".join(texts)
    try:
        url = "https://translate.googleapis.com/translate_a/single?client=gtx&sl=en&tl=" + target_lang + "&dt=t&q=" + urllib.parse.quote(combined_text)
        req = urllib.request.Request(url, headers={'User-Agent': 'Mozilla/5.0'})
        with urllib.request.urlopen(req, timeout=15) as response:
            ...
            if len(translated_lines) == len(texts):
                ...
                print(f"Warning: Line mismatch for {target_lang}: expected {len(texts)}, got {len(translated_lines)}. Using individual fallback.")
        return [translate_individual(t, target_lang) for t in texts]
    except Exception as e:
        print(f"Error batch translating to {target_lang}: {e}. Using individual fallback.")
        return [translate_individual(t, target_lang) for t in texts]
...
def main():
    ...
    for file in files:
        ...
        # 1. Untranslated keys (value == key)
        # 2. Leaked Portuguese translations (value == pt_br[key] and pt_br[key] != key)
        keys_to_translate = []
        for key, val in data.items():
            ...
            is_leaked_pt = False
            if lang != 'pt' and key in pt_br:
                pt_val = pt_br[key]
                if pt_val != key and val == pt_val:
                    is_leaked_pt = True
        ...
        if keys_to_translate:
            ...
            for i in range(0, len(keys_to_translate), chunk_size):
                ...
                print(f"Translating chunk {i // chunk_size + 1} of {(len(keys_to_translate) - 1) // chunk_size + 1} ({len(chunk)} keys)...")
