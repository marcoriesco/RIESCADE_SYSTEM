...
namespace EmulatorLauncher
{
    partial class YabasanshiroGenerator
    {
        ...
        private void CreateControllerConfiguration(IniFile ini)
        {
            ...
            if (SystemConfig.isOptSet("yaba_multitap") && !string.IsNullOrEmpty(SystemConfig["yaba_multitap"]))
                _multitap = SystemConfig["yaba_multitap"];
            ...
            for (int i = 1; i < 7; i++)
            {
                ...
                ini.WriteValue("0.9.11", port1Type, "0");
        ...
        private void ConfigureJoystick(IniFile ini, Controller ctrl, int playerindex)
        {
            ...
            if (padlayout == "lr_xz")
            {
                foreach (KeyValuePair<string, InputKey> pair in lr_xz_profile)
                    ini.WriteValue("0.9.11", "Input\\Port\\" + port + "\\Id\\" + controllerId + "\\Controller\\" + cType + "\\Key\\" + pair.Key, GetInputKeyName(ctrl, pair.Value, index));
            }
