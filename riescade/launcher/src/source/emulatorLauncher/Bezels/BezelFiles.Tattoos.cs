        ...
        public static string GetTattooImage(string inputPng, string outputPng, string emulator)
        {
            ...
            try
            {
                if (Program.SystemConfig.getOptBoolean("tattoo"))
                {
                    ...
                    tattooFile = Path.Combine(Program.AppConfig.GetFullPath("user"), "tattoos", "games", system, gameName + ".png");
                    if (!File.Exists(tattooFile))
                        tattooFile = Path.Combine(Program.AppConfig.GetFullPath("user"), "tattoos", "default", system + ".png");
                    if (!File.Exists(tattooFile))
                        tattooFile = Path.Combine(Program.AppConfig.GetFullPath("riescade"), "system", "tattoos", "games", system, gameName + ".png");
                    if (!File.Exists(tattooFile))
                        tattooFile = Path.Combine(Program.AppConfig.GetFullPath("riescade"), "system", "tattoos", "default", tattooName);
                    if (!File.Exists(tattooFile))
                    {
