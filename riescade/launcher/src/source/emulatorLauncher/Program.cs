...
namespace EmulatorLauncher
{
    static class Program
    {
        ...
        /// <summary>
        /// Import systems.json to retrieve emulators
        /// </summary>
        ...
        public static EsSystems EsSystems
        {
            get
            {
                if (_esSystems == null)
                {
                    _esSystems = EsSystems.Load(Path.Combine(Program.AppConfig.GetFullPath("home"), "systems.json"));
                }
        ...
        public static EsSaveStates EsSaveStates
        {
            get
            {
                if (_esSaveStates == null)
                    _esSaveStates = EsSaveStates.Load(Path.Combine(Program.AppConfig.GetFullPath("home"), "savestates.json"));
        ...
        public static bool HasEsSaveStates
        {
            get
            {
                return File.Exists(Path.Combine(Program.AppConfig.GetFullPath("home"), "savestates.json"));
            }
        }
        ...
        static void Main(string[] args)
        {
            ...
            SystemConfig = ConfigFile.LoadEmulationStationSettings(Path.Combine(Program.AppConfig.GetFullPath("home"), "settings.json"));
            ...
            if (generator != null)
            {
                ...
                try
                {
                    Features = EsFeatures.Load(Path.Combine(Program.AppConfig.GetFullPath("home"), "features.json"));
                }
                catch (Exception ex)
                {                    
                    WriteCustomErrorFile("[Error] features.json is invalid :\r\n" + ex.Message); // Delete custom err
                ...
                using (var screenResolution = ScreenResolution.Parse(SystemConfig["videomode"]))
                {
                    ...
                    if (path != null)
                    {
                        ...
                        if (generator.UseEsPadToKey)
                            mapping = PadToKey.Load(Path.Combine(Program.AppConfig.GetFullPath("home"), "padtokey.json"));
        ...
        private static InputConfig[] LoadControllerConfiguration(string[] args)
        {
            ...
            try
            {
                var inputConfig = EsInput.Load(Path.Combine(Program.AppConfig.GetFullPath("home"), "input.json"));
                if (inputConfig != null)
                {
